...
from runs.pattern_scenarios import SCENARIOS
...
def _run_agent(scenario: dict, client) -> dict:
    ...
    for _ in range(scenario.get("max_steps", MAX_STEPS)):
        start = time.time()
        try:
        ...
        if step >= scenario.get("max_steps", MAX_STEPS):
            break
...
def _pick_tool(scenario: dict, step: int, tools: list) -> str:
    ...
    if pattern == "cyclic_redundancy" and tools:
        return tools[step % len(tools)]
    if pattern == "query_entropy_collapse":
