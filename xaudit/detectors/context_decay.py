...
Detects performance context decay — agent efficiency declining over
the course of a session, detected via CUSUM changepoint analysis.

Method: CUSUM changepoint detection on sliding efficiency windows.
Field:  Statistical process control, change detection theory.
"""
    ...
    def detect(self, trace: AgentTrace) -> DetectorResult:
        if trace.total_steps < self.window_size * 2:
            return self._insufficient(
                f"Need at least {self.window_size * 2} steps for context decay analysis."
            )
        ...
        if len(efficiencies) < 2:
            return self._insufficient("Insufficient windows for context decay analysis.")
        ...
        if eff_array.max() == 0.0:
            return DetectorResult(
                ...
                interpretation="Agent had 0% success rate across all windows — complete context decay.",
        ...
        if eff_array.min() == 1.0:
            return self._insufficient(
                "Agent had 100% success rate throughout — no context decay."
            )
        ...
        for eff in efficiencies:
            # Lower CUSUM: detects downward shift (context decay)
