"""
Strategy Persistence Detector

Detects strategy persistence — agent continuing a failing strategy
past the rational pivot point instead of adapting.

Method: Failure event tracking and pivot latency calculation.
Field:  Empirical behavioral analysis, persistence modeling.
"""
...
class StrategyPersistenceDetector(BaseDetector):
    ...
    def detect(self, trace: AgentTrace) -> DetectorResult:
        if not trace.failures:
            return self._insufficient("No failure events in trace — strategy persistence analysis is not applicable.")

        if trace.total_steps < 6:
            return self._insufficient("Trace too short for strategy persistence analysis.")
        ...
        # If agent never pivoted — maximum persistence
        if strategy_change_step is None:
            ...
            interpretation = (
                ...
                + (
                    "Significant persistence — slow to adapt."
