...
Detects loss of tool diversity over time using Rényi entropy.
A rational agent maintains a diverse set of tools; collapse indicates
premature narrowing of information retrieval.

Method: Rényi entropy (α=2) over sliding windows, with slope analysis.
    ...
    def detect(self, trace: AgentTrace) -> DetectorResult:
        ...
        if len(tool_events) < self.window_size + 1:
            return self._insufficient(
                f"At least {self.window_size + 1} tool events are required for trend analysis."
            )
        ...
        if all(e == 0.0 for e in diversity_values):
            ...
            return DetectorResult(
                ...
                interpretation="Single tool dominates completely. No diversity. Query entropy collapse detected.",
        ...
        if np.isclose(slope, 0.0) or slope > 0:
            ...
            raw_score = abs(slope)
            score = min(raw_score / self.threshold, 1.0)
            detected = bool(score > self.threshold and final_entropy < 0.5 * initial_entropy)
        ...
        interpretation_suffix = (
            "Query entropy collapse pattern detected."
            if detected
            else "Tool diversity relatively stable or expanding — no query entropy collapse detected."
        )
