...
Method: Wasserstein-1 distance between observed retrieval weights
        and uniform baseline distribution.
    ...
    def detect(self, trace: AgentTrace) -> DetectorResult:
        ...
        if len(retrievals) < 2:
            return self._insufficient(
                "Less than 2 retrieval events — primacy_dominance not computable."
            )
        if not llm_calls:
        ...
        return DetectorResult(
            ...
            interpretation=(
                ...
                + (
                    "PrimacyDominance pattern detected."
                    if (score > self.threshold) and is_primacy
                    else "No significant primacy_dominance detected."
                )
            ),
