...
def generate_timeline(trace: AgentTrace, report: dict, output_dir: str) -> str:
    ...
    anchoring = report["detectors"].get("anchoring", {})
    sunk = report["detectors"].get("sunk_cost", {})
    degrad = report["detectors"].get("degradation", {})

    anchor_source_step = None

    if anchoring.get("detected"):
        ev = anchoring.get("evidence", {})
        ...
        ax_main.text(
            first_step + 0.3,
            3.55,
            f"anchoring zone (steps {first_step}–{answer_step})",
    ...
    if biases:
        ...
        if anchoring.get("detected"):
            ...
            insight_parts.append(
                f"anchoring: answer {first_sim:.0%} similar to step-0 retrieval "
