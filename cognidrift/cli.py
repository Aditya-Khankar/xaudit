...
def _analyze_single_file(trace_file: Path, fmt: str, output_dir: str, no_viz: bool, json_only: bool, theme_name: str | None) -> dict | None:
    ...
    from cognidrift.themes import get_theme
    from cognidrift.utils.logger import setup_logger
    from cognidrift.recorder.format_adapters import get_adapter
    from cognidrift.utils.format_detect import detect_format
    from cognidrift.report.builder import build_report
    ...
    try:
        ...
        return {
            ...
            "report_path": f"{output_dir}/behavior_report.json",
            "png_path": png_path,
            "report": report
        }
    except Exception as e:
...
@click.option("--theme", "theme_name", default=None,
              ...
              help="Visual theme. Default: amber. Saved with: cognidrift config set theme <name>")
def analyze(trace_path, fmt, output_dir, no_viz, json_only, debug, theme_name):
    """Analyze agent traces for behavioral bias patterns.
    ...
    if len(trace_files) <= 5:
        ...
        for trace_file in trace_files:
            ...
            if res and not json_only:
                ...
                t.print_error(f"Analysis failed for {trace_file.name}. Check cognidrift.log for details.")
    else:
        ...
        t.console.print(f"\n[bold green]✓ Batch Complete:[/bold green] {success_count}/{len(trace_files)} traces analyzed.")
...
def _print_results(report: dict, theme) -> None:
    ...
    theme.print_score(score, biases)

    # Print interpretation for detected biases
    for bias_name in biases:
        interp = detectors.get(bias_name, {}).get("interpretation", "")
        if interp:
            theme.print_bias_detail(bias_name, interp)
...
@click.option("--theme", default=None,
              ...
              help="Visual theme.")
def demo(theme):
    """Run Cognidrift on a pre-packaged sample trace."""
    ...
    try:
        with resources.as_file(sample_ref) as sample_path:
            if not sample_path.exists():
                t.print_error("Sample traces not found. Reinstall Cognidrift: pip install -e .")
                sys.exit(1)

            with open(sample_path) as f:
                raw = json.load(f)
    except Exception:
        t.print_error("Could not load sample trace. Please reinstall Cognidrift.")
        sys.exit(1)
    ...
    t.print_output_paths(png_path, f"{output_dir}behavior_report.json")
    t.print_footer()
...
def config(action, key, value):
    """Manage Cognidrift configuration.

    Examples:
        cognidrift config set theme amber
        cognidrift config set anchoring.threshold 0.5
        cognidrift config get theme
        cognidrift config reset
    """
    from cognidrift.themes import (
...
def themes():
    ...
    from cognidrift.themes import THEME_COLORS, get_theme
    from rich.console import Console
    console = Console()
    ...
    for name in THEME_COLORS.keys():
        ...
        t.print_score(0.81, ["anchoring"])
        console.print()

    console.print("Set a theme:")
    console.print("  cognidrift config set theme amber\n")
    console.print("Use once:")
    console.print("  cognidrift demo --theme nebula\n")
