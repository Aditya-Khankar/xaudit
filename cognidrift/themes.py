...
CONFIG_PATH = Path.home() / ".cognidrift" / "config.json"
...
def _render_banner(theme_name: ThemeName) -> str:
    ...
    try:
        import pyfiglet
        return pyfiglet.figlet_format("Cognidrift", font=font)
    except Exception:
...
def _supports_color() -> bool:
    ...
    # Manual override
    if os.environ.get("COGNIDRIFT_FORCE_COLOR"):
        return True
...
def load_config() -> dict:
    """Load global Cognidrift configuration."""
...
def save_config(config: dict) -> None:
    """Save global Cognidrift configuration."""
...
def load_theme_config() -> ThemeName:
    """Load saved theme from ~/.cognidrift/config.json."""
    config = load_config()
...
def save_theme_config(theme_name: ThemeName) -> None:
    """Save theme choice to ~/.cognidrift/config.json."""
    config = load_config()
